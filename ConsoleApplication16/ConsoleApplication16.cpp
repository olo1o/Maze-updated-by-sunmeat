	...
	while (true)
	{
		if (_kbhit()) // если было нажатие на клавиши пользователем
		{
			...
			if (code == KeyCode::ENTER && strike >= 1)
			{
				...
				{
					maze[position.Y][position.X + 1] = MazeObject::CROSS;
					COORD temp;
					temp.X = position.X + 1;
					temp.Y = position.Y;
			...
			if (code == KeyCode::ENTER && strike >= 1)
			{
				...
				{
					maze[position.Y + 1][position.X] = MazeObject::CROSS;
					COORD temp;
					temp.X = position.X;
					temp.Y = position.Y + 1;
					SetConsoleCursorPosition(h, temp);
			...
			if (code == KeyCode::ENTER && strike >= 1)
			{
				...
				{
					maze[position.Y + 1][position.X + 1] = MazeObject::CROSS;
					COORD temp;
					temp.X = position.X + 1;
					temp.Y = position.Y + 1;
					SetConsoleCursorPosition(h, temp);
			...
			if (energy <= 0)
			{
				int answer = MessageBoxA(0, "У тебя не осталось энергии!\n\nхотите начать сначала?", "ПОРАЖЕНИЕ!", MB_YESNO);
				system("cls");
				if (answer == IDYES)
				{
					main();
